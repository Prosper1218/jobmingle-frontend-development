...
export default function UpdateProfileForm() {
   ...
   const [updateProfile, {isLoading: isGeneratingProfile, error}] = useUpdateProfileMutation();
   ...
   const handleSelected = (category: string, option: any): void => {
      setSelectedPreferences((prev) => {
         ...
         if (category === "interests") {
            ...
            return {
               ...prev,
               interests: isSelected ? prev.interests.filter((item) => item !== option) : [...prev.interests, option],
            };
         }
   ...
   const handleUpdateProfile = async (e: React.FormEvent) => {
      ...
      const base64Image = selectedPreferences.image ? await convertFileToBase64(selectedPreferences.image) : null;
   ...
   return (
      <div className="w-[90%] flex flex-col gap-5 py-10 my-10 px-5 md:px-20 m-auto justify-center border border-stone-950 rounded-md">
         {step === 0 && (
            <div>
               <h2 className="text-xl text-stone-800 text-center font-bold mb-5">Select your Interests</h2>
                  ...
                  {userPreferences.interests.map((option) => (
                     <input
                        ...
                        className={`w-full list-none py-3 cursor-pointer pl-2 border-black-100 border-solid border-[1px] sora rounded-[10px] capitalize ${
                           selectedPreferences.interests.includes(option) ? "text-yellow-500 border-yellow-500" : ""
                        }`}
                        key={option}
         ...
         {step === 1 && (
            <div>
               <h2 className="text-xl text-stone-800 text-center font-bold mb-5">Select your goals</h2>
               <div className="flex flex-col space-y-6 justify-center">
                  {userPreferences.usage.map((option) => (
                     <input
                        ...
                        className={`w-full list-none py-3 cursor-pointer pl-2 border-black-100 border-solid border-[1px] sora rounded-[10px]  ${
                           selectedPreferences.goals === option ? "text-yellow-500 border-yellow-500" : ""
                        }`}
                        key={option}
            ...
            <div className="flex  md:flex-col items-center justify-center gap-5">
               ...
               <div className="relative w-40 h-30 ">
                  <div className="rounded-[50%]- rounded-full overflow-hidden border border-gray-300">
                     {imagePreview ? (
                        <Image src={imagePreview} alt="Profile Picture" width={160} height={160} className="object-cover h-[160px] w-[160px] rounded-full" />
                     ) : (
                        <div className="bg-gray-200 w-full h-[80px] text-center pl-6- flex items-center justify-center text-gray-500">Upload Image</div>
                     )}
                  </div>

                  {/* Edit Icon */}
                  <label htmlFor="profile-picture-input" className="absolute bottom-0 left-2 bg-yellow-600 p-2 rounded-full cursor-pointer">
                  ...
                  <input
                     ...
                     type="file"
                     accept="image/*"
                     onChange={(e) => handleSelected("image", e.target.files ? e.target.files[0] : null)}
                     className="hidden"
                  />
               </div>
            </div>
         ...
         <div className="flex justify-center md:justify-end gap-5">
            ...
            <button
               ...
               onClick={
                  step < Object.keys(userPreferences).length - 1 ? handleNext : handleUpdateProfile
                  // : handleSubmit
               }
            >
               {!isGeneratingProfile ? (
