...
function Header() {
	...
	return (
		<div className=" flex flex-row py-1- items-center  justify-between py-[0.6rem] px-[1rem] md:py-[1.2rem] md:px-[4rem] border-b border-gray-800  bg-white">
			...
			<div className="border-l-4 border-b-2  rounded-lg p-1 border-yellow-600 max-md:translate-x-2 ">
				...
				<h1 className=" md:hidden text-[18px] md:text-xl font-bold translate-x-4 md:translate-x-3 ">
					{user ? user?.firstName + "!" : ""}
				</h1>
			...
			<div className="flex justify-center border-2 border-yellow-600 items-center w-16 h-16 rounded-full overflow-hidden">
				<div className="w-14 h-14 rounded-full overflow-hidden">
					<Image
						src={
							user?.image ? `${IMAGE_URL}/${user?.image}` : defaultProfilePic
						}
